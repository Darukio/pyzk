        ...
        try:
            if self.tcp:
                ...
                try:
                    ...
                    raise ZKNetworkError("timed out in send") from te
                except Exception as send_err:
                    #import logging
                    #logging.error("Error sending data: %s", send_err)
                    raise send_err
                try:
                    data_recv = self.__sock.recv(response_size + 8)
                except timeout as te:
                    raise ZKNetworkError("timed out in recv") from te
                except Exception as recv_err:
                    #import logging
                    #logging.error("Error receiving data: %s", recv_err)
                    raise recv_err
                ...
                try:
                    ...
                    raise ZKNetworkError("timed out in recvfrom") from te
                except Exception as recv_err:
                    #import logging
                    #logging.error("Error receiving data: %s", recv_err)
                    raise recv_err
            ...
            raise ZKNetworkError("timed out") from te
        except Exception as e:
